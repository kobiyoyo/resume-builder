...
const Builder = () => {
  const oldResumeData = JSON.parse(localStorage.getItem('resumeData') as string) as  ResumeData;
  const checkIfResume = {
    personalInfo: {
      name: '',
      email: '',
  ...
  const [resumeData, setResumeData] = useState<ResumeData>(oldResumeData || checkIfResume);
  ...
  const handleExport = () => router.push(`/export?template=${selectedTemplate}`);
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          {activeTab === 'form' ? (
            ...
            <div className="flex flex-col items-center">
              <ResumePreview resumeData={resumeData} disableTemplateSelection={false} setSelectedTemplate={setSelectedTemplate} />
              <button 
